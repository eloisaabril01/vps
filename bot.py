...
WATERMARK = "TindoHost VPS Service"
WELCOME_MESSAGE = "Welcome To TindoHost! Get Started With Us!"
...
async def setup_container(container_id, status_msg, memory, username, vps_id=None, use_custom_image=False):
    ...
    try:
        ...
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("✅ TindoHost VPS setup completed successfully!", ephemeral=True)
        else:
            await status_msg.edit(content="✅ TindoHost VPS setup completed successfully!")
...
async def show_commands(ctx):
    ...
    try:
        embed = discord.Embed(title="🤖 TindoHost VPS Bot Commands", color=discord.Color.blue())
...
async def create_vps_command(ctx, memory: int, cpu: int, disk: int, owner: discord.Member, 
    ...
    try:
        ...
        status_msg = await ctx.send("🚀 Creating TindoHost VPS instance... This may take a few minutes.")
        ...
        await status_msg.edit(content="🔧 Container created. Setting up TindoHost environment...")
        await asyncio.sleep(5)
        ...
        try:
            embed = discord.Embed(title="🎉 TindoHost VPS Creation Successful", color=discord.Color.green())
            ...
            embed.add_field(name="ℹ️ Note", value="This is a TindoHost VPS instance. You can install and configure additional packages as needed.", inline=False)
            
            await owner.send(embed=embed)
            await status_msg.edit(content=f"✅ TindoHost VPS creation successful! VPS has been created for {owner.mention}. Check your DMs for connection details.")
...
async def list_vps(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Your TindoHost VPS Instances", color=discord.Color.blue())
...
async def connect_vps(ctx, token: str):
    ...
    try:
        ...
        embed = discord.Embed(title="TindoHost VPS Connection Details", color=discord.Color.blue())
...
4. You will be connected to your TindoHost VPS
...
""".format(username=vps["username"]), inline=False)
        ...
        await ctx.send("✅ Connection details sent to your DMs! Use the Tmate command to connect to your TindoHost VPS.", ephemeral=True)
...
async def admin_stats(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="TindoHost System Statistics", color=discord.Color.blue())
...
async def global_stats(ctx):
    ...
    try:
        ...
        embed = discord.Emembed(title="TindoHOst Global Usage Statistics", color=discord.Color.blue())
